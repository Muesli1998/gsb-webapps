...
**Trin:** Bygger direkte på opgave 086a (data) og 086b (empiri/regler). Byg-kort — men med et eksplicit
godkendelsespunkt undervejs (farve-/linjekode) før selve visualiseringen bygges. Del 3 af 3 i
opsplitningen af det oprindelige 086-kort.

**Gren:** `arbejde/086c-udvidet-visuelt-kort`, jf. `AGENTS.md`. Forudsætter at 086a og 086b er merget
til `main` (eller er tilgængelige på deres grene, hvis Christoffer vælger at bygge kortet før begge er
færdige — afklar med Christoffer hvilke dele af 086b's output der reelt er klar til brug).

**Baggrund:** Opgave 085 byggede et visuelt kort (artifact) for tre eksempelsider. Christoffer har set
det og præciseret hvordan det skal udvides og forfines — se de fulde detaljer i 086a og 086b, som denne
opgave forbruger output fra. Kortets visuelle sprog er allerede delvist etableret fra 085: kæde af
...
kvalifikationsgrupper à 2 puljer, jf. 086b Mål 1-2). "Finale"/"Bronzekamp"/"5.-8. plads"-noder er en
fjerde nodetype (ikke niveau, ikke spilletidsside) og hænger som en forgrening af den pulje de hører
til, ikke som deres eget trin i rækkefølgekæden.
...
2. Udvid det eksisterende visuelle kort (artifact fra opgave 085) til at dække alle 33 regioner, med:
   ...
   (c) bekræftet oprykning/nedrykning, vist som en tydelig, farvet pil — KUN hvor 086b fandt konkret
       databelæg eller citerbar regeltekst, aldrig som antagelse. Marker forskelligt om belægget er
       regeltekst, empiri, eller begge;
...
- `work/loeste/086a-*.md`, `work/loeste/086b-*.md` (når afsluttet).
- `work/loeste/085-niveau-fra-rekkefoelge-og-skabelon.md` og det publicerede visuelle kort (artifact)
  bygget derfra i denne samtale — samme visuelle grundsprog skal videreføres.

## Afgrænsning

**Må røres:** nyt/udvidet undersøgelsesdokument (`statistik/results/086c-*`), ny/udvidet
visualiserings-HTML under `statistik/results/` (følg samme konvention som opgave 085 etablerede, hvis
en sådan findes), et idébank-dokument for at notere fremtidsvisionen (se Baggrund).

**Må ikke røres:** `statistik/data/*.db` (kun læses — dette kort skriver ikke til nogen database, det
forbruger kun 086a/086b's allerede gemte output), `apps/netlify-prod/`, `kampsystem/`,
...
Ingen forbindelse i kortet uden et konkret citerbart belæg fra 086a/086b.
...
Ingen andre skøn — resten følger direkte af 086a/086b's fund.

## Ved tvivl

Er det uklart hvordan en bestemt forbindelse skal vises fordi 086b's fund for det pågældende niveau var
