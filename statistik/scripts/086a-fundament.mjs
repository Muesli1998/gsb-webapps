...
const report = { generatedAt: new Date().toISOString(), m1, m2, m3: { regions_requested: 33, calls: calls.length, successful_calls: calls.filter((c) => c.http_status === 200).length, errors: calls.filter((c) => c.error).length, distinct_clubs: registryRows.size, registry_rows_written: registryRows.size, registry_rows_in_db: registryCount, home_region_values: [...registryRows.values()].filter((r) => r.region_id != null).length, calls }, note: 'Mål 1 og 2 bruger kun allerede gemte rådata. Mål 3 bruger udelukkende SearchClubInfo; ingen GetLeagueStanding- eller Nembadminton-kald er foretaget. SearchClubInfo-svaret har ikke et home-region-felt; region_id gemmes derfor som NULL i stedet for at fejlmærke lookup-konteksten som hjemmeregion.' };
const m3 = report.m3;
