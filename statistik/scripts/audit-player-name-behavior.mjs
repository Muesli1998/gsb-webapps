...
const rerun = new Set(['Lasse Bjerregaard Kirt','Norr Bagge Køhler','Pelle Emil Jessing Schjøtt','Kasper Gorm','Lasse Friberg Andersen','Sebastian Larsen Lund']);
const out=[];
for (const p of top) {
 const rows=db.prepare(`SELECT tm.external_match_id,tm.round_date,tm.season_id,c.age_group_id,c.league_raw,c.name_raw competition_name,t.name_raw team
 ...
 if (rerun.has(p.name_raw)) out.push({name:p.name_raw,matches:p.matches,ageGroups:[...new Set(rows.map(x=>x.age_group_id))],seasons:[...new Set(rows.map(x=>x.season_id))],sameDate:sameDate.map(x=>({...x,rowType:`${x.league_raw} | ${x.competition_name}`}))});
}
